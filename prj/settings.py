...
DEFAULT_PATHS = {
    "$PYPROJECTS": ["$DRIVE:\\PProjects\\PyProjects"],
    "$PYPROJECTS_TEST": ["$DRIVE:\\PProjects\\PyProjects_test"],
    "$CURPYPROJECTS": ["$PRJPATH\\..", "$PYPROJECTS"],
    "$C_OR_TILDE": ["C:"] if _IS_WINDOWS else ["~"],
    "$PPATH": ["$P_IN", "$CURPYPROJECTS\\$P_IN"],
    "$DPATH": ["$D_IN", "$DRIVE:\\PProjects\\PData\\$PRJNAME\\$D_IN"],
    "$MDPATH": ["$MD_IN", "$PRJPATH\\myData\\$MD_IN", "$CURPYPROJECTS\\myData\\$MD_IN"],
    "$SRCDIR": ["$PRJPATH\\$SD_IN"],
    "$EXTLIB": ["$EL_IN", "$PRJPATH\\$EL_IN", "$CURPYPROJECTS\\$EL_IN"],
